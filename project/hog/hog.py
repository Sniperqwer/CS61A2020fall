...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        while True:
            ...
            score0 += take_turn(n0, score1, dice)
            say = say(score0, score1)
            if score0 >= goal:
                return score0, score1
            if extra_turn(score0, score1) == 0:
                break
        while True:
            if score1 >= goal:
                return score0, score1
            n1 = strategy1(score1, score0)
            score1 += take_turn(n1, score0, dice)
            say = say(score0, score1)
            if score1 >= goal:
                return score0, score1
            if extra_turn(score1, score0) == 0:
                break
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if score - last_score > maxn:
            maxn = score - last_score
            print(f'{maxn} point(s)! The most yet for Player {who}')
        return announce_highest(who, score, maxn)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_aver = 0.0
    for i in range(1, 11):
        aver = make_averaged(roll_dice, trials_count)
        ans = aver(i, dice)
        if ans > max_aver:
            max_aver = ans
            max_num = i
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # return 6  # Replace this statement
    cut_off = 7.704
    return extra_turn_strategy(score, opponent_score, cut_off, num_rolls=6)
    # END PROBLEM 12
